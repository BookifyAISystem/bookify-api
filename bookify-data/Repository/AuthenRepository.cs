...
namespace bookify_data.Repository
{
    public class AuthenRepository : IAuthenRepository
    {
        ...
        public async Task<string> LoginGoogle(GoogleLoginModel model)
        {
            // Tìm kiếm user theo email (không phân biệt hoa thường)
            ...
            if (user == null)
            {
                ...
                // Sau khi tạo mới, nạp lại thông tin user kèm Role nếu cần
                user = await _dbcontext.Accounts.Include(u => u.Role)
                            .SingleOrDefaultAsync(u => u.AccountId == user.AccountId);
            }

            // Tạo các claim dựa trên thông tin user
